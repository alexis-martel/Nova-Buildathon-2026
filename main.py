...
def compute(data, baseline):
    ...
    rel_theta_power = feature_extraction.get_relative_theta_power(
        data,
        baseline,
        frontal_channels=["Fz"],
        freq_bands={
...
def test_loop():
    ...
    baseline = feature_extraction.get_baseline(
        baseline_eeg,
        frontal_channels=["Fz"],
        freq_bands={
            "delta": [0.5, 4],
    ...
    for i in range(5):
        ...
        print(
            bold(
                f"Patient test score: {acc_score}\tHigher effort than baseline: {high_effort}"
            )
        )
        # Level-change logic
